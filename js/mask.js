 ...
 var AdMacker = function() {
     ...
     var files = Tool.$('#file');
     files.addEventListener('change', function(e) { //change 事件监听
         getfile(e);
     });
     ...
     function drawImg(type, change, img, left, top, imgwidth, imgheight) { //有背景图片的动作
     ...
     function checkValue() {
         ...
         // 画布
         if (!checkFormat('adWidth', regex.reg1, tips[0], 640)) return false;
         if (!checkFormat('adHeight', regex.reg1, tips[0], 100)) return false;
     ...
     for (var i = 0, k = inputs.length; i < k; i++) {
         if (inputs[i].type != 'button' && inputs[i].type != 'file') {
             inputs[i].onchange = (function(i) {
                 return function() {
                     if (inputs[i].id == 'adWidth') {
                         var width = Tool.$('#adWidth').value;
                         var ad = document.getElementById("adMaker");
                         ad.width = width;
                     }
                     if (inputs[i].id == 'adHeight') {
                         var height = Tool.$('#adHeight').value;
                         var ad = document.getElementById("adMaker");
                         ad.height = height;
                     }
                     checkValue();
                     if (typeof image != 'undefined') {
                         drawImg('fill', 'clear', image, iLeft, iTop, image.width * imgScale, image.height * imgScale);
                     } else {
                         drawImg('fill', 'clear');
                     }
                 }
     ...
     for (var t = 0, k = selects.length; t < k; t++) {
         selects[t].onchange = (function(i) {
             return function() {
                 checkValue();
                 if (typeof image != 'undefined') {
