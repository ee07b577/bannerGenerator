...
var AdMacker = function() {
    ...
    function getfile() { //获取图片文件
        ...
        if (file.files.length === 0) { //如果没有传图片~创建画布继续执行
            //颜色点击事件
            createCanvas();
        } else {
    ...
    function drawtext(left, top, imgwidth, imgheight) {//没有背景图片的动作
    ...
    function drawImg(img, left, top, imgwidth, imgheight) { //有背景图片的动作
        var canvas = Tool.$('#adMaker');
        var context = canvas.getContext('2d');
        //向画布上绘制图片
    ...
    for(var t = 0,k = selects.length;t<k;t++){
        selects[t].onchange = createCanvas;
    }
   Tool.$('#putOut').onclick = putOut;
